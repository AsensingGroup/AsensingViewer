#ifndef VTKA2PacketInterpreter_h
#define VTKA2PacketInterpreter_h
...
class VTK_EXPORT vtkA2PacketInterpreter : public vtkLidarPacketInterpreter
{
public:
  static vtkA2PacketInterpreter* New();
  vtkTypeMacro(vtkA2PacketInterpreter, vtkLidarPacketInterpreter)
...
protected:
  ...
  vtkA2PacketInterpreter();
  ~vtkA2PacketInterpreter();

private:
  vtkA2PacketInterpreter(const vtkA2PacketInterpreter&) = delete;
  void operator=(const vtkA2PacketInterpreter&) = delete;
  ...
  float elevation_offset_[A2_CHANNEL_NUM] = {0};
  float azimuth_offset_[A2_CHANNEL_NUM] = {0};
  bool channels[A2_CHANNEL_NUM] = {0};
...
struct AsensingSpecificFrameInformation : public SpecificFrameInformation
{
  ...
  bool IsNewFrame(int index, uint32_t newFrameID)
  {
    ...
    if(prev_LastFrameID == 0 || newFrameID == 0)
    {
      return false;
    }
...
#endif // VTKA2PacketInterpreter_h
